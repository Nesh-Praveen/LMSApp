...
#import "LMSForgotPassword.h"

@interface LMSForgotPassword (){

    
    __weak IBOutlet UITextField *emailTxtFiled;
}

@end

@implementation LMSForgotPassword
...
- (IBAction)resetBtnTapped:(id)sender {
    ...
    [emailTxtFiled resignFirstResponder];
    
    if ([self isValid]) {
        
        NSString *uName = emailTxtFiled.text;
        NSString *loginService = [NSString stringWithFormat:kLMS_ForgotPasswordSer,uName];
...
- (BOOL)isValid{
    ...
    BOOL isValid = NO;
    if (YES == [LMSCommon isEmptyString:emailTxtFiled.text]){
        
        [emailTxtFiled becomeFirstResponder];
        [emailTxtFiled setKeyboardType:UIKeyboardTypeDefault];
